...
private extension TSI.Requests.GetLocalizedEvents.Response.Model.Parser {
	...
	static func createEvent (_ jsonArray: [JSON], _ keysIndices: KeysIndices, _ json: JSON) throws -> TSI.Requests.GetLocalizedEvents.Response.Model.Event {
		...
		let event = TSI.Requests.GetLocalizedEvents.Response.Model.Event(
			time: time,
			groups: groups,
			lecturer: lecturer,
			rooms: rooms,
